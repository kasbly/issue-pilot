...
note_start=$(date +%s)
out=$(bash -c "$run" </dev/null 2>>"$STATE_DIR/announce.log" || true)
bash "$PKG_DIR/bin/cost-log.sh" note release "$note_start" "$run" || true  # no stdin: codex exec would wait on it
